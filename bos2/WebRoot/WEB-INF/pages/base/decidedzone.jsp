		...
		$('#grid').datagrid( {
			...
			onDblClickRow : function(index,row){
					$('#association_subarea').datagrid( {
						...
						rownumbers : true,
						striped : true,
						url : "${pageContext.request.contextPath }/subareaAction_linkSubarea.action?condition="+encodeURI(encodeURI(row.name)),
						columns : [ [{
							field : 'id',
							title : '分拣编号',
							width : 120,
...
<body class="easyui-layout" style="visibility:hidden;">
	...
	<div class="easyui-window" title="定区添加修改" id="addDecidedzoneWindow" collapsible="false" minimizable="false" maximizable="false" style="top:20px;left:200px">
		...
		<div style="overflow:auto;padding:5px;" border="false">
			<form id="addDecidedzone" action="{pageContext.request.contextPath}/decidedzoneAction_addDecidedzone.action" method="post">
				<table class="table-edit" width="80%" align="center">
					...
					<tr>
						<td>定区名称</td>
						<td><input type="text" id="decidedzoneName" name="name" class="easyui-validatebox" required="true"/></td>
					</tr>
					<tr>
						<td>选择负责人</td>
						<td>
							<input name="bcStaff.id" id="staffCombobox"/>
							<script type="text/javascript">
							     $(function(){
							         $("#staffCombobox").combobox({
								             valueField:'id',
								             textField:'name',
								             url:'${pageContext.request.contextPath }/staffAction_staffToCombobox.action'
							                 });
							     });
							</script>  
						</td>
					...
					<tr height="300">
						...
						<td>
							<table id="subareaGrid" border="false" style="width:300px;height:300px" >
							<script type="text/javascript">
							     $(function(){
							          ...
							          $("#decidedzoneName").focusout(function(){
							             var condition=$(this).val();
										             if(condition){
											                 $.get("{pageContext.request.contextPath}/subareaAction_linkSubarea.action?condition="+condition,function(data){
											                     if(data){
											                         $("#subareaGrid").datagrid("loadData",data);
											                     }
											                 },"json");
										                 }
										             else{
										                 $.messager.alert("提示信息","请输入定区信息","warning");
										             }
							         });
							     });
							</script>
